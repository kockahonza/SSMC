...
function analyze_single_mmps_step_!(
    ...
    if length(all_steadystates) == 3
        s1p = nospace_sol_check_physical(all_steadystates[1])
        s2p = nospace_sol_check_physical(all_steadystates[2])
        if s2p
            s1s = nospace_sol_check_stable(mmpns, all_steadystates[1])
            s2s = nospace_sol_check_stable(mmpns, all_steadystates[2])
            if !s1s || s2s
                @show s1s s2s
            end
        end
