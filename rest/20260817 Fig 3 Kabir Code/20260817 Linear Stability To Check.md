...
**Quasi-steady-state assumption:** the resources equilibrate fast, so we set $\partial_t \rho = \partial_t\sigma = 0$ while keeping their diffusion. This is the key simplification: it lets us solve for $\rho$ and $\sigma$ in terms of $\nu$, and reduces a $3\times3$ eigenvalue problem to a single scalar equation.

---

## Step 3: Solve for $\rho$ (the external resource response)
...
- The second inherits $\rho$, which is negative. More cells deplete $r_{\text{ext}}$, so *less* gets leaked into $r_\times$. This is an inhibitory contribution travelling through the byproduct.
...
Linearise. The perturbation $\nu$ multiplies the bracket — but **the bracket vanishes at the fixed point** (Step 1). So that term dies, and only $n^*$ times the bracket's perturbation survives:
...
**At $k\to\infty$ with $D_n=0$:** every term vanishes, $\lambda\to 0$. This is Step 5's consequence: screen out the resources and $n$ has nothing left to do.
...
**The key observation:** $I_2$ decays as $k^{-4}$, one power faster than the other two. It is a *double* Lorentzian, because that inhibitory signal must pass through **two** screened resource fields in sequence — depletion of $r_{\text{ext}}$, propagated into $r_\times$. In real space this means its range is $\sqrt{\ell_{\text{ext}}^2 + \ell_\times^2}$, strictly longer than the activator's $\ell_\times$, **for any diffusivities whatsoever**.

So the separation of scales here is **topological, not diffusive** — it comes from path length through the metabolic chain, not from $D_{\text{ext}}$ exceeding $D_\times$. Dropping $I_2$ from the large-$k$ balance:
