...
class SpatialMiCRMParams:
    """Spatial Modified MiCRM parameters (1D or 2D).

    Wraps a :class:`MiCRMParams` with diffusion coefficients and a grid spec.
    The same container serves both dimensionalities: 1D solvers use ``dx``
    only, 2D solvers use ``dx`` (x-axis) and ``dy`` (y-axis). Diffusion is
    isotropic per species (``Ds[i]`` applies on every axis). The grid size is
    inferred from the initial condition at solve time, not stored here.
    ...
    dy    : float       grid spacing along y (2D only; defaults to ``dx``)
    bc    : {"periodic", "closed"}
    """
    ...
    def __post_init__(self) -> None:
        ...
        self.dy = self.dx if self.dy is None else float(self.dy)
