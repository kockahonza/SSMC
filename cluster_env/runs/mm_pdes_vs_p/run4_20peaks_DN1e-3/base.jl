################################################################################
# run4_20peaks_DN1e-3 - run3 in a domain five times smaller
#
# Identical to run3_100peaks_DN1e-3 in every parameter but one: each domain is
# sized to hold 20 peaks of its fastest growing mode instead of 100. Same K=5,
# DN=1e-3, l=0.999, m=c=d=1, DI=1, the same 5 log spaced p, 10 repeats,
# sN=2500, same perturbation, same T=1e12 and solver settings.
#
# What it is for: run3 found the coarsening outcome to depend very strongly on
# p - essentially no coarsening at p<=0.1, 100->80 at p=0.316, and 100->6 at
# p=1. Shrinking the box 5x is the same finite-size test run2 was to run1. If
# the arrest is at a preferred *spacing*, the final counts should scale down
# with L and land near a fifth of run3's; if the box is what sets them, they
# will not.
#
# The p=1 end is the interesting one. run3 coarsened to a spacing of ~30.5,
# which is most of this run's entire domain (L=36.6 at p=1), so those rows may
# well coarsen all the way to one or two peaks - the single-peak state run1 and
# run2 never reached.
#
# One bonus of the smaller domain: sN stays 2500, so dx is 5x finer than run3's
# and every p is better resolved. run3's p=0.01 was already converged (102-103
# peaks over an 8x dx range), so this run has margin to spare.
#
...
`min_peaks=20` is the only thing that differs from run3; K=5 and DN=1e-3 carry
over unchanged. `make_setup` refuses to write a setup whose
...
The `seed` differs from run3's on purpose. sN and epsilon are unchanged, so
