...
function marsland_initialization_v2(S, M;
    SA=5, MA=5, q=0.9, c1=1.0, muc=10,
    fs=0.45, fw=0.45, sparsity=0.2
)
    @assert S > 5 && M > 5 "S and M must be at least 6 each"

    # let us begin by assigning the whole array c0
    cdist = base10_lognormal(0.0, 0.001)
    c = Matrix{Float64}(undef, S, M)
    for ci in eachindex(c)
        c[ci] = rand(cdist)
    end
    ...
    mdist = base10_lognormal(0.0, 0.001)
    m = rand(mdist, S)
...
function v2main1()
    ...
    @tasks for ri in 1:num_runs
        params[ri] = marsland_initialization_v2(S, M)
    end
