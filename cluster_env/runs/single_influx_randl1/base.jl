...
function do_Klalpha_run(Ks, lalphas, num_repeats;
    DN=0.,
    rsg_kwargs=(;),
    T=1e8,
    # ODE solver params
    ...
    num_runs = length(Ks) * length(lalphas) * num_repeats
    ...
    for Ki in 1:length(Ks)
        for lalphai in 1:length(lalphas)
            K = Ks[Ki]
            lalpha = lalphas[lalphai]
            rsg = get_si_sampler_for_paper(K, Beta(lalpha, 1.), DN; rsg_kwargs...)
            ...
            @show (K, lalpha, countmap(codes[rows]))
            flush(stdout)
...
function make_count_matrices(df)
    ...
    for code in codes
        mat = matrices[code] = zeros(Int, numKs, numlalphas)
        for r in eachrow(df)
            if r.codes == code
                mat[r.Kis, r.lalphais] += 1
            end
        end
    end
    matrices
...
function main1()
    ...
    lalphas = 10 .^ range(0., 2., 30)

    df, metadata = do_Klalpha_run(Ks, lalphas, 150;
        T=1e6,
        maxtime=120,
        rsg_kwargs=(;)
    )
